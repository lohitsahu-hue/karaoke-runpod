...
# Pin yt-dlp to latest for YouTube bot-detection bypass
RUN pip install --no-cache-dir runpod "yt-dlp>=2024.12.01" demucs requests audio-separator==0.18.0 onnxruntime && pip cache purge
...
# Pre-download hdemucs_mmi model (faster alternative)
RUN python3 -c "import torch; from demucs.pretrained import get_model; get_model('hdemucs_mmi')"
